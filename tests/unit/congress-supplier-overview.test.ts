...
import { openDatabase, publishedCongressSupplierOverview, startRun } from '@senadotracker/db';
...
test('hub unificado soma parlamentar+institucional sem deduplicar por valor e rankeia por dimensão', () => {
  ...
  try {
    ...
    assert.deepEqual(result.categories,[{category:'Passagens',value:23000},{category:'Telefonia',value:7000}]);
    ...
    assert.equal(result.parliamentaryCategoryAvailability,'available');
    assert.equal(result.institutionalCategoryAvailability,'unavailable');
  } finally { db.close(); }
});
