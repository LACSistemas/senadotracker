...
test('migrações são repetíveis e chaves estrangeiras/identidades duplicadas são protegidas', async () => {
  ...
  try {
    ...
    assert.equal(db.prepare('SELECT count(*) AS n FROM schema_migrations').get()!.n, 20);
  } finally { db.close(); }
