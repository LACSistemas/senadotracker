...
export function publishLegislation(db:DatabaseSync,runId:string,source:Source,year:number,deliberations:Deliberation[],votes:LegislativeVote[]){transaction(db,()=>{const run=db.prepare('SELECT status FROM ingestion_runs WHERE id=? AND source=?').get(runId,source);if(!run||run.status!=='running'||!deliberations.length)throw new Error('Lote legislativo inválido');const ds=new Set(deliberations.map(d=>d.externalId));if(ds.size!==deliberations.length||votes.some(v=>!ds.has(v.deliberationId)))throw new Error('Voto sem deliberação ou deliberação duplicada');const now=new Date().toISOString();db.prepare('INSERT INTO legislative_batches VALUES (?,?,?,?,?,?)').run(runId,source,year,now,deliberations.length,votes.length);const di=db.prepare('INSERT INTO deliberations VALUES (?,?,?,?,?,?,?)');for(const d of deliberations)di.run(runId,source,d.externalId,year,d.date,d.rawId,JSON.stringify(d));const vi=db.prepare('INSERT INTO legislative_votes VALUES (?,?,?,?,?,?)');for(const v of votes)vi.run(runId,source,v.deliberationId,v.externalId,v.rawId,JSON.stringify(v));const nominal=new Set(votes.filter(v=>countsAsParticipation(source,v.vote)).map(v=>v.deliberationId)),ni=db.prepare('INSERT INTO nominal_deliberations VALUES (?,?)');for(const deliberationId of nominal)ni.run(runId,deliberationId);db.prepare('INSERT INTO active_legislative_publications VALUES (?,?,?) ON CONFLICT(source,year) DO UPDATE SET batch_id=excluded.batch_id').run(source,year,runId);db.prepare("UPDATE ingestion_runs SET status='published',finished_at=?,expected_count=?,roster_complete=1,reconciliation_json=? WHERE id=?").run(now,votes.length,JSON.stringify({year,deliberations:deliberations.length,votes:votes.length}),runId);db.prepare('DELETE FROM job_locks WHERE run_id=?').run(runId)})}
...
export function publishActivity(db:DatabaseSync,runId:string,source:Source,scope:string,proposals:Proposal[],authors:ProposalAuthor[],appointments:LegislativeAppointment[],laws:LawLink[]){transaction(db,()=>{const run=db.prepare('SELECT status FROM ingestion_runs WHERE id=? AND source=?').get(runId,source);if(!run||run.status!=='running')throw new Error('Lote de atuação inválido');const ids=new Set(proposals.map(p=>p.externalId));if(ids.size!==proposals.length||authors.some(a=>!ids.has(a.proposalId))||laws.some(l=>!ids.has(l.proposalId)))throw new Error('Relação legislativa órfã ou proposição duplicada');for(const p of proposals){const expected=proposalFunctionalGroup(p.type);if(p.functionalGroup!==expected)throw new Error(`Grupo funcional divergente para ${p.type}: ${p.functionalGroup} != ${expected}`)}const now=new Date().toISOString();db.prepare('INSERT INTO activity_batches VALUES (?,?,?,?,?,?,?,?)').run(runId,source,scope,now,proposals.length,authors.length,appointments.length,laws.length);const pi=db.prepare('INSERT INTO proposals(batch_id,source,external_id,raw_id,payload,grupo_atribuido) VALUES (?,?,?,?,?,?)');for(const p of proposals)pi.run(runId,source,p.externalId,p.rawId,JSON.stringify(p),p.functionalGroup);const ai=db.prepare('INSERT INTO proposal_authors VALUES (?,?,?,?,?,?)');for(const [i,a] of authors.entries())ai.run(runId,a.proposalId,`${a.externalId??a.name}:${a.order??i}`,a.externalId,a.rawId,JSON.stringify(a));const xi=db.prepare('INSERT INTO legislative_appointments VALUES (?,?,?,?,?,?,?)');for(const a of appointments)xi.run(runId,a.externalId,source,a.personExternalId,a.kind,a.rawId,JSON.stringify(a));const li=db.prepare('INSERT INTO law_links VALUES (?,?,?,?,?)');for(const l of laws)li.run(runId,l.proposalId,l.lawId,l.rawId,JSON.stringify(l));db.prepare('INSERT INTO active_activity_publications VALUES (?,?,?) ON CONFLICT(source,scope) DO UPDATE SET batch_id=excluded.batch_id').run(source,scope,runId);db.prepare("UPDATE ingestion_runs SET status='published',finished_at=?,expected_count=?,roster_complete=1,reconciliation_json=? WHERE id=?").run(now,proposals.length,JSON.stringify({scope,proposals:proposals.length,authors:authors.length,appointments:appointments.length,laws:laws.length}),runId);db.prepare('DELETE FROM job_locks WHERE run_id=?').run(runId)})}
export function publishedActivity(db:DatabaseSync,source:Source,externalId:string){const proposals=db.prepare(`SELECT DISTINCT p.payload FROM proposals p JOIN active_activity_publications a ON a.batch_id=p.batch_id JOIN proposal_authors x ON x.batch_id=p.batch_id AND x.proposal_id=p.external_id WHERE a.source=? AND x.person_external_id=? ORDER BY json_extract(p.payload,'$.presentedAt') DESC LIMIT 100`).all(source,externalId).map(r=>JSON.parse(String(r.payload)) as Proposal);const appointments=db.prepare(`SELECT x.payload FROM legislative_appointments x JOIN active_activity_publications a ON a.batch_id=x.batch_id WHERE a.source=? AND x.person_external_id=? ORDER BY COALESCE(json_extract(x.payload,'$.start'),'') DESC LIMIT 100`).all(source,externalId).map(r=>JSON.parse(String(r.payload)) as LegislativeAppointment);const laws=db.prepare(`SELECT l.payload FROM law_links l JOIN active_activity_publications a ON a.batch_id=l.batch_id JOIN proposal_authors x ON x.batch_id=l.batch_id AND x.proposal_id=l.proposal_id WHERE a.source=? AND x.person_external_id=? ORDER BY l.law_id LIMIT 100`).all(source,externalId).map(r=>JSON.parse(String(r.payload)) as LawLink);const proposalTotal=Number(db.prepare(`SELECT count(DISTINCT x.proposal_id) value FROM proposal_authors x JOIN active_activity_publications a ON a.batch_id=x.batch_id WHERE a.source=? AND x.person_external_id=?`).get(source,externalId)?.value??0),appointmentTotal=Number(db.prepare(`SELECT count(*) value FROM legislative_appointments x JOIN active_activity_publications a ON a.batch_id=x.batch_id WHERE a.source=? AND x.person_external_id=? AND x.kind='rapporteurship'`).get(source,externalId)?.value??0),lawTotal=Number(db.prepare(`SELECT count(DISTINCT l.proposal_id) value FROM law_links l JOIN active_activity_publications a ON a.batch_id=l.batch_id JOIN proposal_authors x ON x.batch_id=l.batch_id AND x.proposal_id=l.proposal_id WHERE a.source=? AND x.person_external_id=?`).get(source,externalId)?.value??0);return{proposals,appointments,laws,proposalTotal,appointmentTotal,lawTotal}}
...
export function publishedParticipation(db:DatabaseSync,source:Source,externalId:string,year=2026){const cutoff=reportingCutoff(year);const profile=getPublished(db,source,externalId);const periods=profile?.exercises??[];const batch=db.prepare('SELECT b.* FROM presence_batches b JOIN active_presence_publications a ON a.batch_id=b.id WHERE a.source=? AND a.year=?').get(source,year);const officialPresence=source==='camara'?db.prepare('SELECT * FROM chamber_official_presence WHERE external_id=? AND year=?').get(externalId,year):null;const sessionRows=batch&&!officialPresence?db.prepare('SELECT payload FROM legislative_sessions WHERE batch_id=? AND eligible=1 AND substr(date,1,10)<=? ORDER BY date').all(String(batch.id),cutoff).map(r=>JSON.parse(String(r.payload)) as LegislativeSession):[];const eligibleSessions=sessionRows.filter(s=>within(s.date,periods));const attended=batch&&!officialPresence?new Set(db.prepare('SELECT session_id FROM attendance WHERE batch_id=? AND external_id=?').all(String(batch.id),externalId).map(r=>String(r.session_id))):new Set<string>();const attendanceDays=new Map<string,boolean>();for(const session of eligibleSessions)attendanceDays.set(session.date.slice(0,10),Boolean(attendanceDays.get(session.date.slice(0,10)))||attended.has(session.externalId));const attendanceNumerator=[...attendanceDays.values()].filter(Boolean).length;const deliberations=db.prepare(`SELECT d.external_id,d.date FROM deliberations d JOIN active_legislative_publications a ON a.batch_id=d.batch_id WHERE a.source=? AND a.year=? AND substr(d.date,1,10)<=? AND json_extract(d.payload,'$.chamberBody')='PLEN'`).all(source,year,cutoff).map(r=>({externalId:String(r.external_id),date:String(r.date)})).filter(d=>within(d.date,periods));const accepted=source==='camara'?"'sim','nao','abstencao','votou','secreto','obstrucao'":"'sim','nao','abstencao','votou','secreto'";const nominalIds=new Set(db.prepare(`SELECT n.deliberation_id FROM nominal_deliberations n JOIN active_legislative_publications a ON a.batch_id=n.batch_id WHERE a.source=? AND a.year=?`).all(source,year).map(row=>String(row.deliberation_id)));const voted=new Set(db.prepare(`SELECT v.deliberation_id FROM legislative_votes v JOIN active_legislative_publications a ON a.batch_id=v.batch_id WHERE a.source=? AND a.year=? AND v.external_id=? AND search_text(json_extract(v.payload,'$.vote')) IN (${accepted})`).all(source,year,externalId).map(row=>String(row.deliberation_id)));const eligibleDeliberations=deliberations.filter(d=>nominalIds.has(d.externalId));const participationNumerator=eligibleDeliberations.filter(d=>voted.has(d.externalId)).length;return{year,presence:{availability:batch?String(batch.availability):'unavailable',numerator:officialPresence?Number(officialPresence.days_present):batch?.availability==='available'?attendanceNumerator:null,denominator:officialPresence?Number(officialPresence.days_total):batch?.availability==='available'?attendanceDays.size:null,note:officialPresence?'Dias de Plenário conforme o relatório individual oficial da Câmara.':batch?(source==='camara'?'Dias únicos derivados do lote de eventos; justificativas podem ter cobertura parcial.':String(batch.note)):'Lote de presença não publicado'},participation:{availability:eligibleDeliberations.length?'available':'unavailable',numerator:eligibleDeliberations.length?participationNumerator:null,denominator:eligibleDeliberations.length||null,note:'Votações nominais plenárias durante períodos de exercício; não mede presença.'}}}
export function publishComplement(db:DatabaseSync,runId:string,source:Source,scope:string,records:LegislativeComplement[],reconciliation:unknown){transaction(db,()=>{const run=db.prepare('SELECT status FROM ingestion_runs WHERE id=? AND source=?').get(runId,source);if(!run||run.status!=='running'||new Set(records.map(r=>r.externalKey)).size!==records.length)throw new Error('Lote complementar inválido ou duplicado');for(const r of records){if(r.source!==source)throw new Error('Fonte complementar divergente');if(!db.prepare('SELECT id FROM raw_objects WHERE id=? AND run_id=?').get(r.rawId,runId))throw new Error('Complemento sem evidência do lote')}const now=new Date().toISOString();db.prepare('INSERT INTO complement_batches VALUES (?,?,?,?,?,?)').run(runId,source,scope,now,records.length,JSON.stringify(reconciliation));const insert=db.prepare('INSERT INTO legislative_complements VALUES (?,?,?,?,?,?,?,?,?,?,?)'),enriched=db.prepare('INSERT INTO proposal_enrichment_items VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'),activate=db.prepare('INSERT INTO active_proposal_enrichment_publications VALUES (?,?,?,?,?) ON CONFLICT(source,proposal_id,kind) DO UPDATE SET batch_id=excluded.batch_id,published_at=excluded.published_at');const activeKinds=new Set<string>();for(const r of records){insert.run(runId,source,r.externalKey,r.kind,r.personExternalId,r.proposalId,r.deliberationId,r.bodyId,r.occurredAt,r.rawId,JSON.stringify(r));if(r.proposalId){let value:Record<string,unknown>={};try{value=JSON.parse(r.value??'{}') as Record<string,unknown>}catch{}enriched.run(runId,source,r.proposalId,r.kind,r.externalKey,String(value.relatedId??'')||null,r.deliberationId,r.personExternalId,r.bodyId,r.occurredAt,String(value.code??'')||null,r.label,String(value.description??value.dispatch??'')||null,r.officialUrl,String(value.inheritedFromProposalId??'')||null,r.rawId,JSON.stringify(value));activeKinds.add(`${r.proposalId}\u0000${r.kind}`)}}for(const value of activeKinds){const [proposalId,kind]=value.split('\u0000');activate.run(source,proposalId!,kind!,runId,now)}db.prepare('INSERT INTO active_complement_publications VALUES (?,?,?) ON CONFLICT(source,scope) DO UPDATE SET batch_id=excluded.batch_id').run(source,scope,runId);db.prepare("UPDATE ingestion_runs SET status='published',finished_at=?,expected_count=?,roster_complete=1,reconciliation_json=? WHERE id=?").run(now,records.length,JSON.stringify(reconciliation),runId);db.prepare('DELETE FROM job_locks WHERE run_id=?').run(runId)})}
...
export function publishedVotesPage(db:DatabaseSync,source:Source,externalId:string,page=1,pageSize=10){
  if(!Number.isSafeInteger(page)||page<1||!Number.isSafeInteger(pageSize)||pageSize<1||pageSize>100)throw new Error('Paginação de votos inválida');
  const cutoff=reportingCutoff(new Date().getFullYear()),from=`FROM legislative_votes v JOIN active_legislative_publications a ON a.batch_id=v.batch_id JOIN deliberations d ON d.batch_id=v.batch_id AND d.external_id=v.deliberation_id JOIN legislative_batches b ON b.id=v.batch_id JOIN raw_objects r ON r.id=v.raw_id WHERE v.source=? AND v.external_id=? AND substr(d.date,1,10)<=?`,params=[source,externalId,cutoff] as const;
  const total=Number(db.prepare(`SELECT count(*) value ${from}`).get(...params)?.value??0),rows=db.prepare(`SELECT v.payload vote,d.payload deliberation,r.url,r.fetched_at,r.sha256,b.published_at,v.batch_id ${from} ORDER BY d.date DESC,v.deliberation_id LIMIT ? OFFSET ?`).all(...params,pageSize,(page-1)*pageSize);
  return{total,page,pageSize,items:rows.map(r=>({vote:JSON.parse(String(r.vote)) as LegislativeVote,deliberation:JSON.parse(String(r.deliberation)) as Deliberation,origin:{url:String(r.url),fetchedAt:String(r.fetched_at),sha256:String(r.sha256),publishedAt:String(r.published_at),batchId:String(r.batch_id)}}))};
}
