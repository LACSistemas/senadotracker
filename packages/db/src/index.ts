...
export function publishedVotes(db:DatabaseSync,source:Source,externalId:string){const rows=db.prepare(`SELECT v.payload vote,d.payload deliberation,r.url,r.fetched_at,r.sha256,b.published_at,v.batch_id FROM legislative_votes v JOIN active_legislative_publications a ON a.batch_id=v.batch_id JOIN deliberations d ON d.batch_id=v.batch_id AND d.external_id=v.deliberation_id JOIN legislative_batches b ON b.id=v.batch_id JOIN raw_objects r ON r.id=v.raw_id WHERE v.source=? AND v.external_id=? ORDER BY d.date DESC,v.deliberation_id`).all(source,externalId);return rows.map(r=>({vote:JSON.parse(String(r.vote)) as LegislativeVote,deliberation:JSON.parse(String(r.deliberation)) as Deliberation,origin:{url:String(r.url),fetchedAt:String(r.fetched_at),sha256:String(r.sha256),publishedAt:String(r.published_at),batchId:String(r.batch_id)}}))}
...
export function publishedParticipation(db:DatabaseSync,source:Source,externalId:string,year=2026){const profile=getPublished(db,source,externalId);const periods=profile?.exercises??[];const batch=db.prepare('SELECT b.* FROM presence_batches b JOIN active_presence_publications a ON a.batch_id=b.id WHERE a.source=? AND a.year=?').get(source,year);const sessionRows=batch?db.prepare('SELECT payload FROM legislative_sessions WHERE batch_id=? AND eligible=1 ORDER BY date').all(String(batch.id)).map(r=>JSON.parse(String(r.payload)) as LegislativeSession):[];const eligibleSessions=sessionRows.filter(s=>within(s.date,periods));const attended=batch?new Set(db.prepare('SELECT session_id FROM attendance WHERE batch_id=? AND external_id=?').all(String(batch.id),externalId).map(r=>String(r.session_id))):new Set<string>();const attendanceNumerator=eligibleSessions.filter(s=>attended.has(s.externalId)).length;const deliberations=db.prepare(`SELECT d.external_id,d.date,d.payload FROM deliberations d JOIN active_legislative_publications a ON a.batch_id=d.batch_id WHERE a.source=? AND a.year=?`).all(source,year).map(r=>JSON.parse(String(r.payload)) as Deliberation).filter(d=>d.chamberBody==='PLEN'&&within(d.date,periods));const voted=new Set(db.prepare(`SELECT v.deliberation_id FROM legislative_votes v JOIN active_legislative_publications a ON a.batch_id=v.batch_id WHERE a.source=? AND a.year=? AND v.external_id=?`).all(source,year,externalId).map(r=>String(r.deliberation_id)));const participationNumerator=deliberations.filter(d=>voted.has(d.externalId)).length;return{year,presence:{availability:batch?String(batch.availability):'unavailable',numerator:batch?.availability==='available'?attendanceNumerator:null,denominator:batch?.availability==='available'?eligibleSessions.length:null,note:batch?String(batch.note):'Lote de presença não publicado'},participation:{availability:deliberations.length?'available':'unavailable',numerator:deliberations.length?participationNumerator:null,denominator:deliberations.length||null,note:'Votações nominais plenárias durante períodos de exercício; não mede presença.'}}}
