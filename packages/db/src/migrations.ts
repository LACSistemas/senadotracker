export const migrations = [
  ...
  { version: 38, sql: `
    ALTER TABLE financial_movements ADD COLUMN expense_element TEXT;
    ALTER TABLE financial_movements ADD COLUMN expense_nature TEXT;
    ALTER TABLE financial_movements ADD COLUMN ug TEXT;
    ALTER TABLE financial_movements ADD COLUMN favored_document TEXT;
    ALTER TABLE financial_movements ADD COLUMN favored_name TEXT;
    ALTER TABLE financial_movements ADD COLUMN siafi_document TEXT;
    ALTER TABLE financial_movements ADD COLUMN supplier_classification TEXT CHECK(supplier_classification IS NULL OR supplier_classification IN ('supplier_related_confirmed','supplier_related_conditional','non_supplier','unknown'));
    CREATE INDEX financial_movements_camara_execution ON financial_movements(source_system,movement_year,phase,expense_element,ug);
  ` },
  { version: 39, sql: `ALTER TABLE financial_movements ADD COLUMN expense_group TEXT;` },
  { version: 40, sql: `ALTER TABLE financial_movements ADD COLUMN expense_nature_raw TEXT; ALTER TABLE financial_movements ADD COLUMN expense_nature_key TEXT;` },
  { version: 41, sql: `CREATE TABLE chamber_financial_pages (window_start TEXT NOT NULL, window_end TEXT NOT NULL, page INTEGER NOT NULL, official_rows_seen INTEGER NOT NULL DEFAULT 0, unique_rows_seen INTEGER NOT NULL DEFAULT 0, raw_persisted INTEGER NOT NULL DEFAULT 0, payments_persisted INTEGER NOT NULL DEFAULT 0, last_committed_at TEXT, status TEXT NOT NULL CHECK(status IN ('pending','fetched','persisted','complete','failed')), error TEXT, PRIMARY KEY(window_start,window_end,page)); CREATE INDEX chamber_financial_pages_status ON chamber_financial_pages(status,window_start,window_end,page);` },

  { version: 42, sql: `CREATE TABLE institutional_publication_membership (revision_scope TEXT NOT NULL, source_system TEXT NOT NULL, source_movement_id TEXT NOT NULL, period_start TEXT NOT NULL, period_end TEXT NOT NULL, included INTEGER NOT NULL CHECK(included IN (0,1)), reason TEXT NOT NULL, created_at TEXT NOT NULL, PRIMARY KEY(revision_scope,source_system,source_movement_id)); CREATE INDEX institutional_publication_membership_lookup ON institutional_publication_membership(revision_scope,source_system,included);` },
  { version: 43, sql: `ALTER TABLE supplier_aggregate_revisions ADD COLUMN chamber_expense_publication_id TEXT; ALTER TABLE supplier_aggregate_revisions ADD COLUMN senate_expense_publication_id TEXT; ALTER TABLE supplier_aggregate_revisions ADD COLUMN chamber_institutional_publication_revision TEXT; ALTER TABLE supplier_aggregate_revisions ADD COLUMN senate_institutional_publication_revision TEXT; ALTER TABLE supplier_aggregate_revisions ADD COLUMN cutoff_start TEXT; ALTER TABLE supplier_aggregate_revisions ADD COLUMN cutoff_end TEXT;` },
  { version: 44, sql: `CREATE TABLE supplier_category_revisions (revision_id TEXT PRIMARY KEY REFERENCES supplier_aggregate_revisions(id),status TEXT NOT NULL CHECK(status IN ('building','reconciled','published','superseded')),cutoff_start TEXT,cutoff_end TEXT,chamber_institutional_revision TEXT,senate_institutional_revision TEXT,chamber_expense_publication_id TEXT,senate_expense_publication_id TEXT,created_at TEXT NOT NULL,published_at TEXT,canonical_records INTEGER NOT NULL DEFAULT 0,classified_records INTEGER NOT NULL DEFAULT 0,unclassified_records INTEGER NOT NULL DEFAULT 0,canonical_value_scaled INTEGER NOT NULL DEFAULT 0,classified_value_scaled INTEGER NOT NULL DEFAULT 0,unclassified_value_scaled INTEGER NOT NULL DEFAULT 0,category_coverage_records REAL,category_coverage_value REAL); CREATE TABLE active_supplier_category_revision (singleton INTEGER PRIMARY KEY CHECK(singleton=1),revision_id TEXT NOT NULL REFERENCES supplier_category_revisions(revision_id)); CREATE TABLE supplier_category_snapshot (revision_id TEXT NOT NULL REFERENCES supplier_category_revisions(revision_id),universe TEXT NOT NULL,institution TEXT NOT NULL,category_key TEXT NOT NULL,category_label TEXT NOT NULL,category_source TEXT NOT NULL,category_level TEXT NOT NULL,value_scaled INTEGER NOT NULL,payment_count INTEGER NOT NULL,supplier_count INTEGER NOT NULL,pct_total REAL NOT NULL,PRIMARY KEY(revision_id,universe,institution,category_key)); CREATE INDEX supplier_category_snapshot_lookup ON supplier_category_snapshot(revision_id,institution,universe,value_scaled DESC);` },
  { version: 45, sql: `ALTER TABLE supplier_category_revisions ADD COLUMN enrichment_attempt_complete INTEGER NOT NULL DEFAULT 0;` },
  { version: 46, sql: `ALTER TABLE financial_movements ADD COLUMN classification_source TEXT; ALTER TABLE financial_movements ADD COLUMN classification_level TEXT; ALTER TABLE financial_movements ADD COLUMN official_financial_classification TEXT; CREATE TABLE senate_category_enrichment_checkpoints (ne_key TEXT PRIMARY KEY,status TEXT NOT NULL CHECK(status IN ('classified','unavailable','pending_retry','failed')),attempts INTEGER NOT NULL DEFAULT 0,last_error TEXT,last_attempted_at TEXT,source_record_id TEXT);` },
  { version: 47, sql: `ALTER TABLE financial_movements ADD COLUMN enrichment_status TEXT CHECK(enrichment_status IS NULL OR enrichment_status IN ('classified','unavailable','retryable_failed','terminal_failed')); ALTER TABLE financial_movements ADD COLUMN enrichment_attempt_count INTEGER NOT NULL DEFAULT 0; ALTER TABLE financial_movements ADD COLUMN enrichment_last_attempt_at TEXT; ALTER TABLE financial_movements ADD COLUMN enrichment_last_error TEXT; ALTER TABLE financial_movements ADD COLUMN enrichment_last_http_status INTEGER; ALTER TABLE financial_movements ADD COLUMN enrichment_last_success_at TEXT; CREATE INDEX financial_movements_enrichment_status ON financial_movements(source_system,enrichment_status);` },
  { version: 48, sql: `ALTER TABLE supplier_category_revisions ADD COLUMN enrichment_state_json TEXT NOT NULL DEFAULT '{}';` },
  { version: 49, sql: `CREATE TABLE legislative_bodies(id TEXT PRIMARY KEY,source TEXT NOT NULL,external_id TEXT NOT NULL,sigla TEXT,name TEXT NOT NULL,short_name TEXT,type_raw TEXT,type_normalized TEXT NOT NULL DEFAULT 'UNKNOWN',house TEXT,active INTEGER,parent_body_id TEXT,source_url TEXT,raw_id TEXT,first_seen_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(source,external_id)); CREATE TABLE commission_meetings(id TEXT PRIMARY KEY,source TEXT NOT NULL,external_id TEXT NOT NULL,title TEXT,description TEXT,meeting_type_raw TEXT,meeting_type_normalized TEXT,status_raw TEXT,status_normalized TEXT,scheduled_start_at TEXT,scheduled_end_at TEXT,actual_start_at TEXT,actual_end_at TEXT,location TEXT,room TEXT,source_url TEXT,video_url TEXT,raw_id TEXT,first_seen_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(source,external_id)); CREATE TABLE commission_meeting_bodies(meeting_id TEXT NOT NULL,body_id TEXT NOT NULL,relationship_type TEXT,is_primary INTEGER,raw_id TEXT,created_at TEXT NOT NULL,PRIMARY KEY(meeting_id,body_id)); CREATE TABLE commission_meeting_versions(id TEXT PRIMARY KEY,meeting_id TEXT NOT NULL,payload_hash TEXT NOT NULL,payload TEXT NOT NULL CHECK(json_valid(payload)),raw_id TEXT,observed_at TEXT NOT NULL,UNIQUE(meeting_id,payload_hash)); CREATE TABLE commission_agenda_items(id TEXT PRIMARY KEY,source TEXT NOT NULL,external_id TEXT NOT NULL,meeting_id TEXT NOT NULL,proposal_source TEXT,proposal_id TEXT,order_raw TEXT,order_normalized INTEGER,item_type_raw TEXT,title TEXT,description TEXT,regime TEXT,result_raw TEXT,source_url TEXT,raw_id TEXT,active INTEGER NOT NULL DEFAULT 1,first_seen_at TEXT NOT NULL,last_seen_at TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,removed_at TEXT,UNIQUE(source,external_id)); CREATE TABLE commission_agenda_item_versions(id TEXT PRIMARY KEY,item_id TEXT NOT NULL,payload_hash TEXT NOT NULL,payload TEXT NOT NULL CHECK(json_valid(payload)),raw_id TEXT,active INTEGER NOT NULL,observed_at TEXT NOT NULL,UNIQUE(item_id,payload_hash)); CREATE INDEX commission_meetings_date ON commission_meetings(scheduled_start_at); CREATE INDEX commission_items_meeting ON commission_agenda_items(meeting_id,active); CREATE INDEX commission_items_proposal ON commission_agenda_items(proposal_source,proposal_id);` },
  { version: 50, sql: `ALTER TABLE commission_meetings ADD COLUMN scheduled_date TEXT; CREATE INDEX commission_meetings_scheduled_date ON commission_meetings(scheduled_date);` },
  { version: 51, sql: `CREATE TABLE commission_membership_snapshots (body_id TEXT NOT NULL REFERENCES legislative_bodies(id), source TEXT NOT NULL, body_external_id TEXT NOT NULL, person_external_id TEXT NOT NULL, role_raw TEXT, party TEXT, uf TEXT, legislature TEXT, start_date TEXT, end_date TEXT, status TEXT, observed_at TEXT NOT NULL, raw_id TEXT, payload_hash TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1, PRIMARY KEY(body_id,person_external_id,role_raw,payload_hash)); CREATE INDEX commission_membership_current ON commission_membership_snapshots(body_id,active,observed_at);` },
