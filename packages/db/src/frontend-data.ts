...
export function publishedCabinetPanorama(db:DatabaseSync,source:Source){
  ...
  const staffTotal=staffCounts.reduce((sum,value)=>sum+value,0),staffMean=staffCounts.length?staffTotal/staffCounts.length:null;
  if(source==='camara'){
    ...
    return{source,period:String(year),staff:distribution(staffCounts),staffTotal,staffMean,financial:distribution(totals.map(item=>item.value)),financialMean:totals.length?totals.reduce((sum,item)=>sum+item.value,0)/totals.length:null,sampleSize:totals.length,kind:'budget_spent' as const};
  }
  ...
  return{source,period:competence,staff:distribution(staffCounts),staffTotal,staffMean,financial:distribution([...totals.values()]),financialMean:totals.size?[...totals.values()].reduce((sum,value)=>sum+value,0)/totals.size:null,sampleSize:totals.size,kind:'identified_payroll' as const};
}
