...
import { proposalFunctionalGroup, type LegislativeAppointment, type Proposal, type ProposalAuthor, type Source } from '@senadotracker/domain';
import { saveRaw } from './raw.ts';
import { OfficialHttp } from './http.ts';
import { resolveCamaraPrincipalProposals } from './principal-proposal-resolver.ts';
...
export async function collectActivity(db:DatabaseSync,source:Source,dataDir:string,progress:(s:string)=>void=()=>{}){
 const run=startRun(db,source);try{
  ...
  if(source==='camara'){
   ...
   for(const x of p.data.dados){const status=x.ultimoStatus??{},year=Number(x.ano)||null,number=x.numero==null?null:String(x.numero),type=String(x.siglaTipo??x.descricaoTipo??''),principalProposalId=idFromUri(x.uriPropPrincipal),previousProposalId=idFromUri(x.uriPropAnterior),nextProposalId=idFromUri(x.uriPropPosterior),hasRelationship=Boolean(principalProposalId||previousProposalId||nextProposalId);proposals.push({source,externalId:String(x.id),type,functionalGroup:proposalFunctionalGroup(type),number,year,label:[type,number].filter(Boolean).join(' ')+(year?`/${year}`:''),summary:text(x.ementa),presentedAt:text(x.dataApresentacao),status:text(status.descricaoSituacao),officialUrl:`https://www.camara.leg.br/proposicoesWeb/fichadetramitacao?idProposicao=${x.id}`,rawId:p.rawId,principalProposalId,previousProposalId,nextProposalId,relationshipEvidence:hasRelationship?'official_uri':null,catalogRole:'catalog',discoveredByProposalId:null});const relator=idFromUri(status.uriRelator);if(relator)appointments.push({source,externalId:`relator:${x.id}:${relator}:${status.sequencia??''}`,personExternalId:relator,kind:'rapporteurship',bodyId:status.codOrgao==null?null:String(status.codOrgao),bodyLabel:text(status.siglaOrgao),role:'Relator',start:text(status.data)?.slice(0,10)??null,end:null,status:text(status.descricaoSituacao),proposalId:String(x.id),officialUrl:`https://dadosabertos.camara.leg.br/api/v2/proposicoes/${x.id}`,rawId:p.rawId})}
   ...
   const addProposal=(matter:any,rawId:string)=>{if(!matter?.Codigo)return null;const id=String(matter.Codigo),type=String(matter.Sigla??'');if(!proposalMap.has(id))proposalMap.set(id,{source,externalId:id,type,functionalGroup:proposalFunctionalGroup(type),number:text(matter.Numero),year:Number(matter.Ano)||null,label:String(matter.DescricaoIdentificacao??`${matter.Sigla??''} ${matter.Numero??''}/${matter.Ano??''}`.trim()),summary:text(matter.Ementa),presentedAt:text(matter.Data),status:null,officialUrl:`https://www25.senado.leg.br/web/atividade/materias/-/materia/${id}`,rawId});return id};
  ...
  let principalResolution=null;if(source==='camara')try{principalResolution=await resolveCamaraPrincipalProposals(db,{children:proposals,rawDirectory:resolve(dataDir,'raw'),progress})}catch(error){progress(`camara: filhos publicados; resolução de pais falhou sem bloquear o catálogo: ${error instanceof Error?error.message:String(error)}`)}
  progress(`${source}: ${proposals.length} proposições, ${authors.length} autorias, ${appointments.length} atuações, 0 vínculos normativos explícitos`);return{runId:run,proposals:proposals.length,authors:authors.length,appointments:appointments.length,principalResolution};
