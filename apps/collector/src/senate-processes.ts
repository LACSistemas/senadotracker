...
export function parseSenateProcess(document:unknown,rawId:string,proposalId:string):LegislativeComplement[]{
  ...
  add({externalKey:`process:${proposalId}`,kind:'matter_detail',occurredAt:date(process.dataInicioEfetivo),label:text(process.identificacao)||`MatÃ©ria ${proposalId}`,value:JSON.stringify({processId:process.id,type:text(process.sigla),number:text(process.numero),year:Number(process.ano)||null,summary:text(object(process.conteudo).ementa)||null,presentedAt:date(process.dataInicioEfetivo),status:text(process.situacaoAtual)||null,lastUpdated:process.dthUltimaAtualizacao,law:process.normaGerada??null,officialUrl})});
  ...
  const norm=object(process.normaGerada);if(Object.keys(norm).length)add({externalKey:`resulting-norm:${proposalId}:${text(norm.id)||text(norm.codigo)||text(norm.numero)||'published'}`,kind:'resulting_norm',occurredAt:date(norm.dataPublicacao)||date(norm.dataAssinatura)||date(norm.data),label:text(norm.identificacao)||text(norm.descricao)||'Norma gerada',value:JSON.stringify(norm)});
  for(const [autuationIndex,autuation] of list(process.autuacoes).entries()){
    for(const situation of list(autuation.situacoes)){const id=text(situation.idTipo)||text(situation.sigla),label=text(situation.descricao);if(label)add({externalKey:`process-situation:${proposalId}:${autuationIndex}:${id}:${date(situation.inicio)??''}`,kind:'situation',occurredAt:date(situation.inicio),label,value:date(situation.fim)?`atÃ© ${date(situation.fim)}`:null})}
...
export async function collectSenateProcesses(db:DatabaseSync,options:{year:number;rawDirectory:string;cacheDirectory:string;ids?:string[];dryRun?:boolean;progress?:(message:string)=>void}){
  if(!Number.isInteger(options.year)||options.year<2001||options.year>2100)throw new Error('Ano de processos invÃ¡lido');
  const rows=db.prepare(`SELECT p.external_id id FROM proposals p JOIN active_activity_publications a ON a.batch_id=p.batch_id WHERE a.source='senado' AND substr(json_extract(p.payload,'$.presentedAt'),1,4)=? UNION SELECT json_extract(d.payload,'$.proposalId') id FROM deliberations d JOIN active_legislative_publications a ON a.batch_id=d.batch_id WHERE a.source='senado' AND CAST(strftime('%Y',d.date) AS INTEGER)=? AND json_extract(d.payload,'$.proposalId') IS NOT NULL`).all(String(options.year),options.year);
  ...
  if(!requested.length)throw new Error(`Nenhuma matÃ©ria do Senado encontrada nas votaÃ§Ãµes de ${options.year}`);for(const id of requested)if(!/^\d+$/.test(id))throw new Error(`Identificador de matÃ©ria invÃ¡lido: ${id}`);
  ...
  try{
    ...
    for(let offset=0;offset<requested.length;offset+=concurrency){
      const chunk=requested.slice(offset,offset+concurrency);
      const fetched=await Promise.all(chunk.map(async(id,index)=>{try{
        options.progress?.(`Senado processos ${offset+index+1}/${requested.length}: mat\u00e9ria ${id}`);
        ...
        return{id,detail,process,processDocument};
      }catch(error){return{id,error:error instanceof Error?error.message:String(error)}}}));
      for(const item of fetched){if('error' in item){failures.push({id:item.id,error:item.error});continue}if(!options.dryRun)await saveRaw(db,runId!,options.rawDirectory,item.detail);const rawId=options.dryRun?`dry:senado-process:${item.id}`:await saveRaw(db,runId!,options.rawDirectory,item.process);records.push(...parseSenateProcess(item.processDocument,rawId,item.id))}
    }
