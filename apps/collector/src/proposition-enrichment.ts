...
export function prioritizedProposalIds(db:DatabaseSync,source:Source,year:number,limit:number){
  ...
  return db.prepare(`SELECT p.external_id id,
    CASE WHEN EXISTS(SELECT 1 FROM deliberations d JOIN active_legislative_publications al ON al.batch_id=d.batch_id WHERE d.source=p.source AND json_extract(d.payload,'$.proposalId')=p.external_id) THEN 0 ELSE 1 END priority
